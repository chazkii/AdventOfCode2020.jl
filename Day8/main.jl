...
the only acc +3. It sets the accumulator to 5, and jmp -3 causes the program to 
...
function runprogram()::Int
    ...
    raw_code = readlines(filepath)
    matches = [match(r"^(acc|jmp|nop) ([+\-\d]+)$", line) for line in raw_code]
    instructions = [(m[:1], parse(Int, m[:2])) for m in matches]
    visited = Set{Int}()
    acc = 0
    pointer = 1
    keep_running = true
    while keep_running
        l = length(visited)
        if in(pointer, visited)
            keep_running = false
            return acc
        else
            push!(visited, pointer)
            (opcode, i) = instructions[pointer]
            if opcode == "nop"
                pointer += 1
            elseif opcode == "acc"
                acc += i
...
instruction infinite loop, never leaving that instruction. If you change almost any of 
the jmp instructions, the program will still eventually find another jmp instruction 
and loop forever.
...
# one jmp to nop OR one nop to jmp

function runprogram2()::Int
    filepath = ("./Day8/input.txt")
    raw_code = readlines(filepath)
    matches = [match(r"^(acc|jmp|nop) ([+\-\d]+)$", line) for line in raw_code]
    instructions = [(m[:1], parse(Int, m[:2])) for m in matches]
    opcode_before = "jmp"
    opcode_after = "nop"
    indices = [idx for (idx, i) in enumerate(instructions) if i[:1] == opcode_before]
    last_index = length(instructions)
    for idx_to_flip in indices
        ins = copy(instructions)
        ins[idx_to_flip] = (opcode_after, ins[idx_to_flip][:2])
        visited = Set{Int}()
        acc = 0
        pointer = 1
        keep_running = true
