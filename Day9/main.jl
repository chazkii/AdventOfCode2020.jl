...
"""
using Test
filepath = "./Day9/test-input.txt"
ws = 5 

function findbroken(filepath, ws)::Int
    ...
    while !found
        n = code[i]
        w = code[i - ws:i - 1]
        m = collect(Iterators.product(w, w))
        # delete along d
        ismatch = findmatch(m, n)
        if !ismatch
            found = true
            return n
        end
...
"""
function findweakness(filepath, num)
    code = [parse(Int, l) for l in readlines(filepath)]
    i = 1
    j = 2
    while i < length(code)
