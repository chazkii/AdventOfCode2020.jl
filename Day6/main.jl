...
larger plane, customs declaration forms are distributed to the 
passengers.
...
function sumyesgroups()::Int
    blob = read("./Day6/input.txt", String)
    groups = split(blob, "\n\n")
    return sum(countanyyes(convert(String, group)) for group in groups)
end

function countanyyes(group::String)::Int
    g = replace(group, "\n" => "")
    return length(unique(g))
end

@test countanyyes("abc") == 3
@test countanyyes("a\nb\nc") == 3
@test countanyyes("ab\nbc") == 3
@test countanyyes("a\na\na\na") == 1
@test countanyyes("b") == 1
...
function sumyesgroups2()::Int
    blob = strip(read("./Day6/input.txt", String))
    groups = split(blob, "\n\n")
    return sum(countallyes(convert(String, group)) for group in groups)
end

function countallyes(group::String)::Int
    persons = [convert(String, person) for person in split(group, "\n")]
    return length(intersect(persons...))
end

@test countallyes("abc") == 3
@test countallyes("a\nb\nc") == 0
@test countallyes("ab\nbc") == 1
@test countallyes("a\na\na\na") == 1
@test countallyes("b") == 1
sumyesgroups2()