...
to use every adapter, you'd need to choose them like this:
    ...
    could connect to it directly would need to have a joltage rating of 1, 2, or 3 
    jolts. Of these, only one you have is an adapter rated 1 jolt (difference of 1).
...
outlet, the adapters, and your device. What is the number of 1-jolt differences 
multiplied by the number of 3-jolt differences?
"""
using DataStructures
filepath = "./Day10/test-input.txt"

function countdiffs(filepath)
    sorted_adapters = sort([parse(Int, line) for line in readlines(filepath)])
    prev = 0
    diffs = DefaultDict{Int, Int}(0)
...
many different ways they can be arranged. Every arrangement needs to connect the 
...
"""
function countcombos(filepath)::Int
    sorted_adapters = sort([parse(Int, line) for line in readlines(filepath)])
    pushfirst!(sorted_adapters, 0)
    return countcombos(sorted_adapters)
end

using Memoize
# TODO: figure out better algo


@memoize Dict function countcombos(sorted_adapters::Vector{Int})::Int
    println("first")
    if length(sorted_adapters) == 1
        return 1
    end
    start = first(sorted_adapters)
    rest = sorted_adapters[2:end]
    total = 0
    for (i, r) in enumerate(rest)
        if ((r - start) <= 3)
            total += countcombos(rest[i:end])    
        else
            return total
        end
    end
...
@test countcombos("./Day10/test-input.txt") == 8
@test countcombos("./Day10/test-input-2.txt") == 19208