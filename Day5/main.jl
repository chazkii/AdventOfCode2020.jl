...
function seatid(row::Int, column::Int)::Int
    return row * 8 + column
end

@test seatid(44, 5) == 357

function seatid(bsp::String)::Int
   r_bsp = reverse(bsp)
   row = sum([(x == 'B' ? 1 : 0) << (i - 1) for (i, x) in enumerate(r_bsp[4:10])]) 
   column = sum([(x == 'R' ? 1 : 0) << (i - 1) for (i, x) in enumerate(r_bsp[1:3])])
   return seatid(row, column)
end

@test seatid("BFFFBBFRRR") == 567

function seatids()::Vector
    return [seatid(bsp) for bsp in readlines("Day5/input.txt")]
end

function maxseatid()::Int
...
"""
function myseatid()::Int
    sorted_ids = sort(seatids())
    for (i, seatid) in enumerate(sorted_ids)
        if seatid + 1 != sorted_ids[i + 1]
            return seatid + 1
        end
    end
end