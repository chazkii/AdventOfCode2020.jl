...
None of the currency exchanges seem to have heard of them, but somehow, you'll 
...
"""
using Pipe
entries = [parse(Int, i) for i = readlines("./Day1/input.csv")]
function find2020(input::Vector{Int})::Int
    @pipe entries = input |> copy |> sort
    found = false
    while !found
        lowest = pop!(entries)
        for n in entries
...
function find2020three(input::Vector{Int})::Int
    @pipe entries = input |> copy |> sort
    # shrink entries
    println("$(length(entries)) items before processing")
    lowest = pop!(entries)
    lowest_2 = pop!(entries)
    diff = 2020 - lowest + lowest_2
    entries = filter(p->p < diff, entries)
    println("$(length(entries)) items before processing")
    while len(entries) > 0
        lowest = pop!(entries)
        for pair in Iterators.product(entries, entries)
            total = pair[1] + pair[2] + lowest
            if total == 2020
                return pair[1] * pair[2] * lowest
            elseif total > 2020
                break
            end
        end
